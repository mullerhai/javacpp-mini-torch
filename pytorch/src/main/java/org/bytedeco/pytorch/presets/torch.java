...
public class torch implements LoadEnabled, InfoMapper, BuildEnabled {
    ...
    static String buildPushBackModuleCppText() {
        ...
        // by the Java-side helper org.bytedeco.pytorch.nn.ModuleAsHelper. The C++ side does
        // not need any extra helper code — the typed push_back<Impl>
        // overloads (378 of them) are reached via the existing
        // asXxx() walk in org.bytedeco.pytorch.nn.ModuleAsHelper. This avoids the
    ...
    public static void sharedMap(InfoMap infoMap) {
        infoMap
            ...
            .put(new Info("auto", "c10::reverse_iterator", "ska::flat_hash_map", /*"std::atomic", */"std::conditional", "std::iterator_traits",
                "std::initializer_list", "std::integral_constant", "std::mutex", "std::nullopt_t", "std::reverse_iterator" /*, "std::weak_ptr"*/).skip())
    ...
    public void map(InfoMap infoMap) {
        ...
        infoMap
            ...
            .put(new Info("stack.h").linePatterns("// variadic pop.*", "// copies.*").skip())
...
//            .put(new Info("std::optional<at::DimnameList>").pointerTypes("DimnameListOptional").define())
            ...
            .put(new Info("std::optional<c10::string_view>", "std::optional<std::string_view>").pointerTypes("StringViewOptional").define())
            .put(new Info("std::optional<std::vector<c10::string_view> >", "std::optional<std::vector<std::string_view> >").pointerTypes("StringViewVectorOptional").define())
...
//            .put(new Info("std::variant<int,std::vector<int> >").pointerTypes("IntVectorVariant").define())
        ...
        infoMap
            ...
            .put(new Info("const std::vector<c10::Device>", "std::vector<c10::Device>").pointerTypes("DeviceVector").define())
...
//                                "std::map<std::tuple<void*,void*>,at::DataPtr>").pointerTypes("T_PointerPointer_TDataPtrMap").define())
        ...
        infoMap
            ...
            .put(new Info(
                ...
                "std::unordered_map<torch::autograd::Node*,torch::dynamo::autograd::NodeCall>" // Fix erroneous ns qualification due to a previous `using Node::Node`
...
//            .put(new Info("std::pair<bool,std::variant<int,std::vector<int> > >").pointerTypes("BoolIntVectorVariantPair").define())
            ...
            .put(new Info("std::pair<std::string_view,std::string_view>").pointerTypes("StringViewPair").define())
        ;
        ...
        for (PointerInfo pi : new PointerInfo[]{
            ...
            new PointerInfo("torch::autograd::Node"),
...
//               .put(new Info("torch::data::datasets::MapDataset<torch::data::datasets::MNIST,torch::data::transforms::Stack<torch::data::Example<> > >::OutputBatchType").pointerTypes("Example"))
        ...
        infoMap
            ...
            // InputType/OutputType are "using" typedefs inside Transform<> / TensorTransform<>.
            // Map them to Example so peers don't emit bare unresolved identifiers, but do NOT
            // rely on valueTypes alone for apply(): that drops @ByVal and makes the Generator
            // emit Example* virtual overrides, which do not match
            // `OutputType apply(InputType)` (by value) in base.h / tensor.h.
            .put(new Info(
                "torch::data::transforms::Transform<torch::data::Example<torch::Tensor,torch::Tensor>,torch::data::Example<torch::Tensor,torch::Tensor> >::InputType",
                "torch::data::transforms::TensorTransform<torch::Tensor>::InputType"
            ).valueTypes("Example").pointerTypes("Example"))
            .put(new Info(
                "torch::data::transforms::Transform<torch::data::Example<torch::Tensor,torch::Tensor>,torch::data::Example<torch::Tensor,torch::Tensor> >::OutputType",
                "torch::data::transforms::TensorTransform<torch::Tensor>::OutputType"
            ).valueTypes("Example").pointerTypes("Example"))
            // Force @ByVal + @Cast on apply: C++ takes/returns Example by value via
            // InputType/OutputType typedefs. Without both annotations, Generator emits
            // Example* virtual overrides that do not match base.h / tensor.h and fail
            // with "does not override" / "no viable conversion from Example* to Example".
            // Match TensorExampleTransform's working form:
            //   @ByVal @Cast("...::OutputType*") Example apply(@ByVal @Cast("...::InputType*") Example)
            .put(new Info(
...
//                "  /** Implementation of {@code Transform::apply} that calls {@code operator()}. */\n" +
        ...
        infoMap
            ...
            .put(new Info("torch::nn::Module::register_module<torch::nn::Module>").javaText(
                ...
                "public <M extends Module> M register_module(BytePointer name, M module) { org.bytedeco.pytorch.nn.ModuleAsHelper.remember(module); _register_module(name, module); return module; }\n" +
                ...
                "public <M extends Module> M register_module(String name, M module) { org.bytedeco.pytorch.nn.ModuleAsHelper.remember(module); _register_module(name, module); return module; }"
            ))
        ;
        ...
        infoMap.putFirst(new Info("torch::nn::Module::train")
            ...
            .javaText(
                ...
                "{ org.bytedeco.pytorch.nn.ModuleAsHelper.remember(this); }\n" +
                "/**\n" +
                ...
                "    return org.bytedeco.pytorch.nn.ModuleAsHelper.wrap(this, moduleClass);\n" +
                "}\n" +
                "/**\n" +
        ...
        mapModule(infoMap, "Transformer");
                ...
                // subclasses=false: only Module itself gets a JavaCPP peer for these virtuals.
        // Pure-Java Module subclasses (geometric/rl/llm/...) have no C++ counterpart;
        // generating JavaCPP_Foo : public Foo for them breaks native compile. They
        // still work via ModuleAsHelper.recover on the Module peer allocate path.
        // *Impl layers keep their own torch::nn::*Impl peers from mapModule().
infoMap.put(new Info("torch::nn::Module::forward").javaText(
            ...
            "public @ByVal Tensor forward(@Const @ByRef Tensor input1, @Const @ByRef Tensor input2) { Module m = org.bytedeco.pytorch.nn.ModuleAsHelper.recover(this); return org.bytedeco.pytorch.nn.ModuleAsHelper.hasForwardOverride(m, Tensor.class, Tensor.class) ? m.forward(input1, input2) : _forward_tensor2(input1, input2); }\n" +
            "private native @ByVal @Name(\"forward_tensor3\") @Virtual(subclasses=false, method=\"forward\") Tensor _forward_tensor3(@Const @ByRef Tensor input1, @Const @ByRef Tensor input2, @Const @ByRef Tensor input3);\n" +
            "public @ByVal Tensor forward(@Const @ByRef Tensor input1, @Const @ByRef Tensor input2, @Const @ByRef Tensor input3) { Module m = org.bytedeco.pytorch.nn.ModuleAsHelper.recover(this); return org.bytedeco.pytorch.nn.ModuleAsHelper.hasForwardOverride(m, Tensor.class, Tensor.class, Tensor.class) ? m.forward(input1, input2, input3) : _forward_tensor3(input1, input2, input3); }\n" +
            "public native @ByVal @Name(\"forward_tensor4\") @Virtual(subclasses=false, method=\"forward\") Tensor forward(@Const @ByRef Tensor input1, @Const @ByRef Tensor input2, @Const @ByRef Tensor input3, @Const @ByRef Tensor input4);\n" +
            "public native @ByVal @Name(\"forward_tensor6\") @Virtual(subclasses=false, method=\"forward\") Tensor forward(@Const @ByRef Tensor input1, @Const @ByRef Tensor input2, @Const @ByRef Tensor input3, @Const @ByRef Tensor input4, @Const @ByRef Tensor input5, @Const @ByRef Tensor input6);\n" +
            "public native @ByVal @Name(\"forward_tensor8\") @Virtual(subclasses=false, method=\"forward\") Tensor forward(@Const @ByRef Tensor input1, @Const @ByRef Tensor input2, @Const @ByRef Tensor input3, @Const @ByRef Tensor input4, @Const @ByRef Tensor input5, @Const @ByRef Tensor input6, @Const @ByRef Tensor input7, @Const @ByRef Tensor input8);\n" +
            ...
            "public native @ByVal @Name(\"forward_tuple_tensor_t_tensortensor\") @Virtual(subclasses=false, method=\"forwardT_TensorT_TensorTensor_T_T\") T_TensorT_TensorTensor_T_T forwardT_TensorT_TensorTensor_T_T(@Const @ByRef Tensor input);\n" +
            "public native @ByVal @Name(\"forward_tuple_tensor_t_tensortensor_opt\") @Virtual(subclasses=false, method=\"forwardT_TensorT_TensorTensor_T_T\") T_TensorT_TensorTensor_T_T forwardT_TensorT_TensorTensor_T_T(@Const @ByRef Tensor input, @ByVal T_TensorTensor_TOptional hx_opt);\n" +
            "public native @ByVal @Name(\"forward_tuple_tensor_tensor\") @Virtual(subclasses=false, method=\"forwardT_TensorTensor_T\") T_TensorTensor_T forwardT_TensorTensor_T(@Const @ByRef Tensor input);\n" +
        ...
        infoMap
            ...
            .put(new Info("torch::nn::ModuleListImpl::operator[]").javaText(
                ...
                "public @SharedPtr(\"torch::nn::Module\") @ByVal Module get(@Cast(\"size_t\") long index) { return org.bytedeco.pytorch.nn.ModuleAsHelper.recover(_get(index)); }"
            ))
            ...
            .put(new Info("torch::nn::ModuleDictImpl::operator[]").javaText(
                ...
                "public @SharedPtr(\"torch::nn::Module\") @ByVal Module get(@StdString BytePointer key) { return org.bytedeco.pytorch.nn.ModuleAsHelper.recover(_get(key)); }\n" +
                "private native @SharedPtr(\"torch::nn::Module\") @ByVal @Name(\"operator []\") Module _get(@StdString String key);\n" +
                "public @SharedPtr(\"torch::nn::Module\") @ByVal Module get(@StdString String key) { return org.bytedeco.pytorch.nn.ModuleAsHelper.recover(_get(key)); }"
            ));
...
//            "torch::dynamo::autograd::AutogradCompilerCall::tensor_args",
        ...
        */
        ...
        infoMap.put(new Info(
            ...
            "c10::in_place_t",
            "c10::impl::ptr_to_first_element",
            "c10::impl::op_allowlist_check",
...
//            "torch::autograd::InputMetadata",
            ...
            "torch::jit::StringViewReader", "at::StringView" // Confusion with string_view and @StringView, and doesn't seem to be of any use in API
        ...
        infoMap.put(new Info(
            ...
            "at::impl::MetaBase::set_output_strided",
            "at::impl::MetaBase::set_output_raw_strided",
        ...
        //// Classes kept but passed as generic pointer
               .put(new Info("c10::intrusive_ptr_target", "c10::nullopt", "c10::nullopt_t", "c10::impl::PyObjectSlot",
                   ...
                   "std::pair<c10::SafePyObject,c10::SafePyObject>", "std::stack<std::pair<c10::SafePyObject,c10::SafePyObject> >",
                   ...
                   "c10::ArrayRef<torch::autograd::SavedVariable>",
