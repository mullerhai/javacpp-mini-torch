...
import org.bytedeco.pytorch.dataframe.faiss.Faiss;
import org.bytedeco.pytorch.dataframe.faiss.Index;
import org.bytedeco.pytorch.dataframe.faiss.IndexFlatIP;
import org.bytedeco.pytorch.dataframe.faiss.IndexFlatL2;
import org.bytedeco.pytorch.dataframe.faiss.IndexHNSWFlat;
import org.bytedeco.pytorch.dataframe.faiss.IndexIDMap;
import org.bytedeco.pytorch.dataframe.faiss.IndexIVFPQ;
import org.bytedeco.pytorch.dataframe.faiss.MetricType;
import org.bytedeco.pytorch.dataframe.faiss.SearchResult;
...
import org.bytedeco.pytorch.dataframe.excel.ExcelReader;
import org.bytedeco.pytorch.dataframe.excel.ExcelWriter;
...
import org.bytedeco.pytorch.dataframe.hdf5.Hdf5Writer;
import org.bytedeco.pytorch.dataframe.io.*;
...
import org.bytedeco.pytorch.dataframe.pickle.PandasDataFramePickle;
...
import org.bytedeco.pytorch.dataframe.window.Ewm;
...
import org.bytedeco.pytorch.dataframe.csv.CsvReader;
...
public final class DataFrame implements AutoCloseable, Serializable {
    ...
    public static DataFrame readParquet(String path, int workers) throws Exception {
        return ParallelReader.readParquet(path, workers);
    }
    ...
    public static long streamParquet(String path, int batchRows,
        ...
        return ParallelReader.streamParquet(path, batchRows, consumer);
    }
    ...
    public static DataFrame readPickle(String path) throws Exception {
        return PandasDataFramePickle.load(path);
    }

    public static DataFrame readPickle(String path,
            PickleOptions options) throws Exception {
        return PandasDataFramePickle.load(path, options);
    }

    public void toPickle(String path) throws Exception {
        PandasDataFramePickle.dump(this, path);
    }

    public void toPickle(String path,
            PickleOptions options) throws Exception {
        PandasDataFramePickle.dump(this, path, options);
    }
    ...
    public static DataFrame readImages(String pathOrGlob) throws Exception {
        return MultimodalIO.readImages(pathOrGlob);
    }

    /** Batch-load images with explicit {@link MediaBridge.ImageOptions}. */
    public static DataFrame readImages(String pathOrGlob,
            MediaBridge.ImageOptions opts) throws Exception {
        return MultimodalIO.readImages(pathOrGlob, opts);
    }
    ...
    public static DataFrame readAudio(String pathOrGlob) throws Exception {
        return MultimodalIO.readAudio(pathOrGlob);
    }

    public static DataFrame readAudio(String pathOrGlob, int sampleRate, boolean mono) throws Exception {
        return MultimodalIO.readAudio(pathOrGlob, sampleRate, mono);
    }

    public static DataFrame readAudio(String pathOrGlob,
            MediaBridge.AudioOptions opts) throws Exception {
        return MultimodalIO.readAudio(pathOrGlob, opts, true);
    }

    /**
     * Load video files as frame sequences (Daft {@code read_video}).
     * Real FFmpeg decode via {@link MediaBridge} when available.
    ...
    public static DataFrame readImageFolder(String root) throws Exception {
        return MultimodalIO.readImageFolder(root);
    }
    ...
    public static DataFrame readMultimodal(String pathOrDir) throws Exception {
        return MultimodalIO.readMultimodalDir(pathOrDir);
    }

    /**
     * Build an image DataFrame from in-memory {@link ImageData} cells.
     */
    ...
    public static DataFrame fromOpenCV(String imageCol, List<org.bytedeco.pytorch.Tensor> tensors) {
        return MultimodalIO.fromOpenCVTensors(imageCol, tensors);
    }
    ...
    public static DataFrame fromFFmpegFrames(List<org.bytedeco.pytorch.Tensor> frames, double fps) {
        return MultimodalIO.fromFFmpegFrames("frame", frames, fps);
    }
    ...
    public DataFrame extractVideoFrames(String videoCol, double fps) throws Exception {
        return MultimodalIO.extractVideoFrames(this, videoCol, fps);
    }
    ...
    public DataFrame embedImages(String imageCol, String outCol, int dim) throws Exception {
        return MultimodalIO.embedImages(this, imageCol, outCol, dim);
    }
    ...
    public DataFrame embedAudioCol(String audioCol, String outCol, int dim) throws Exception {
        return MultimodalIO.embedAudio(this, audioCol, outCol, dim);
    }
    ...
    public DataFrame embedVideos(String videoCol, String outCol, int dim) throws Exception {
        return MultimodalIO.embedVideo(this, videoCol, outCol, dim);
    }
    ...
    public org.bytedeco.pytorch.Tensor toVisionBatch(String imageCol) {
        return MediaInterop.toVisionBatch(this, imageCol);
    }
    ...
    public DataFrame transformImages(String imageCol, Object transform) throws Exception {
        return MediaInterop.applyVisionTransform(this, imageCol, transform);
    }
    ...
    public static DataFrame fromEmbeddings(String embCol, float[][] vectors, String modelName) {
        ...
        if (vectors != null) {
            for (float[] v : vectors) {
                int ri = df.addEmptyRow();
                df.set(ri, col, new EmbeddingData(
    ...
    public void writeLanceTraining(String path, String... vectorCols) throws Exception {
        LanceDataset.write(this, path, vectorCols);
    }

    public void writeLanceTraining(String path,
                                   LanceDataset.WriteOptions opts,
    ...
    /**
     * @deprecated Use {@link #writeLanceTraining(String, LanceDataset.WriteOptions, String...)}.
     */
    @Deprecated
    public void writeLance(String path,
                           LanceDataset.WriteOptions opts,
    ...
    public static LanceDataset openLanceTraining(String path)
            throws Exception {
        return LanceDataset.open(path);
    }
     ...
     * </pre>
     */
    public BatchEmbedder embed() {
        return BatchEmbedder.create();
    }

    /** Embed all detectable modality columns with one model. */
    public DataFrame embedAll(String modelId) {
        return BatchEmbedder.embedAll(this, modelId);
    }
    ...
    public DataFrame download(String urlCol, String outCol) throws Exception {
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            try {
                ...
                data.add(new BinaryData("download", bytes));
            } catch (Exception e) {
                data.add(null);
            }
        }
    ...
    public static DataFrame readCsvParallel(String path, int workers) throws Exception {
        return ParallelReader.readCsv(path, workers);
    }

    public static DataFrame readCsvParallel(String path) throws Exception {
        return ParallelReader.readCsv(path);
    }

    public static DataFrame readCsvParallel(String path,
                                            ParallelReader.Options opt)
            throws Exception {
        return ParallelReader.readCsv(path, opt);
    }
    ...
    public static long streamCsv(String path, int workers, long maxChunkBytes,
                                 java.util.function.Consumer<DataFrame> consumer) throws Exception {
        var opt = ParallelReader.Options.defaults()
            .workers(workers)
            .maxChunkBytes(maxChunkBytes);
        return ParallelReader.streamCsv(path, opt, consumer);
    }

    public static long streamCsv(String path, java.util.function.Consumer<DataFrame> consumer)
            throws Exception {
        return ParallelReader.streamCsv(
            path, ParallelReader.Options.defaults(), consumer);
    }
    ...
    public static DataFrame readExcel(String path,
            ExcelOptions options) throws Exception {
        return ExcelReader.read(path, options);
    }

    public static DataFrame readExcel(java.io.InputStream in,
            ExcelOptions options) throws Exception {
        return ExcelReader.read(in, options);
    }

    public static Map<String, DataFrame> readExcelAll(String path) throws Exception {
        return ExcelReader.readAll(path,
            ExcelOptions.defaults());
    }

    public static Map<String, DataFrame> readExcelAll(String path,
            ExcelOptions options) throws Exception {
        return ExcelReader.readAll(path, options);
    }
    ...
    /**
     * Write this DataFrame into a SQLite table with {@link SqlOptions}.
     */
    ...
    public void toSql(java.sql.Connection c, String table) throws Exception {
        SqlWriter.write(this, c, table);
    }

    public void toSql(java.sql.Connection c, String table,
            SqlOptions options) throws Exception {
        SqlWriter.write(this, c, table, options);
    }
     ...
     *
     * @see Redis#connect(String, int)
     */
    public static Redis openRedis() {
        return Redis.connect();
    }

    public static Redis openRedis(String host, int port) {
        return Redis.connect(host, port);
    }

    public static Redis openRedis(
            String host, int port, String password) {
        return Redis.connect(host, port, password);
    }

    /** Parse {@code redis://[:password@]host:port[/db]}. */
    public static Redis openRedis(String uri) {
        return Redis.connectUri(uri);
    }

    /**
     * Write this DataFrame to Redis using {@link RedisOptions}
     ...
     */
    public int toRedis(Redis redis,
                       RedisOptions options) {
        return redis.writeDataFrame(this, options);
    }

    /** HASH layout: one Redis hash per row under {@code prefix + id}. */
    public int toRedis(Redis redis, String prefix) {
        return toRedis(redis, RedisOptions.hash(prefix));
    }
     ...
     */
    public int toRedis(String redisUri, RedisOptions options) {
        try (Redis r =
                     Redis.connectUri(redisUri)) {
            return toRedis(r, options);
        }
    }

    public int toRedis(String redisUri, String prefix, java.time.Duration ttl) {
        return toRedis(redisUri, RedisOptions.hash(prefix, ttl));
    }

    /** Read DataFrame previously stored with {@link #toRedis}. */
    public static DataFrame readRedis(Redis redis,
                                      RedisOptions options) {
        return redis.readDataFrame(options);
    }

    public static DataFrame readRedis(Redis redis, String prefix) {
        return redis.readHash(prefix);
    }

    public static DataFrame readRedisHash(Redis redis, String prefix) {
        return redis.readHash(prefix);
    }

    public static DataFrame readRedisJson(Redis redis, String prefix) {
        return redis.readJsonRows(prefix);
    }

    public static DataFrame readRedisFrame(Redis redis, String key) {
    ...
    public static DataFrame readRedis(String redisUri, String prefix) {
        return readRedis(redisUri, RedisOptions.hash(prefix));
    }
    ...
    public static DataFrame readHdf(String path, String key) throws Exception {
        return Hdf5Reader.read(path, key);
    }

    public static DataFrame readHdf(String path, String key,
            Hdf5Options options) throws Exception {
        return Hdf5Reader.read(path, key, options);
    }

    public void toHdf(String path, String key) throws Exception {
        Hdf5Writer.write(this, path, key);
    }

    public void toHdf(String path, String key,
            Hdf5Options options) throws Exception {
        Hdf5Writer.write(this, path, key, options);
    }
    ...
    public static DataFrame fromTensors(Map<String, Tensor> tensors) {
        ...
        for (Map.Entry<String, Tensor> e : tensors.entrySet()) {
            df.set(ri, e.getKey(),
                TensorData.fromTensor(e.getValue()));
        }
        return df;
    }

    public static DataFrame fromTensorData(String colName, TensorData... cells) {
        DataFrame df = DataFrame.create();
        String name = colName == null ? "tensor" : colName;
        df.addColumn(name, Column.DType.TENSOR);
        if (cells != null) {
            for (TensorData td : cells) {
                int ri = df.addEmptyRow();
                df.set(ri, name, td);
            }
        }
    ...
    public static DataFrame fromNDArray(org.bytedeco.pytorch.data.numpy.NDArray arr, TensorLayout layout) {
        ...
        Tensor t = TensorBridge.toTensor(arr);
    ...
    public Tensor toTensorColumn(String colName) {
        ...
        if (first instanceof TensorData td0) {
            int[] cellShape = td0.getShape();
            int cellSize = td0.size();
            float[] matrix = new float[n * cellSize];
            for (int i = 0; i < n; i++) {
                Object cell = col.get(i);
                float[] row = cell instanceof TensorData td
                    ? td.getData()
                    : TensorBridge.asFloatVector(cell);
                if (row != null) {
        ...
        float[] packed = VectorColumn.pack(col);
        int dim = VectorColumn.dimOf(col);
        if (dim <= 0) {
    ...
    public org.bytedeco.pytorch.data.numpy.NDArray toNDArray(String colName) {
        ...
        if (col.dtype() == Column.DType.VECTOR
            ...
            return TensorBridge.toNDArray(t);
        }
    ...
    private static DataFrame fromTensorRowsAsCells(Tensor t, String colName, boolean unused) {
        ...
        if (shape.length == 0) {
            ...
            int ri = df.addEmptyRow();
            df.set(ri, colName, TensorData.fromTensor(t));
            return df;
        }
        int rows = (int) shape[0];
    ...
    public DataFrame explode(String columnName) {
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            if (v instanceof List) {
                ...
                Object raw = ((DataValue) v).toArrowCompatible();
    ...
    public DataFrame unnest(String structCol) {
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            } else if (v instanceof StructData sd) {
                m = new LinkedHashMap<>(sd.getFieldValues());
            } else if (v != null) {
                m = ComplexCellCodec.asStringMap(v);
            }
            if (m == null) m = Map.of();
    ...
    public static DataFrame readParquetAdvanced(String path,
            ParquetAdvanced.ReadOptions opt) throws Exception {
        return ParquetAdvanced.read(path, opt);
    }

    public static DataFrame readParquet(String path, String... columns) throws Exception {
        return ParquetAdvanced.read(path,
            ParquetAdvanced.ReadOptions.defaults()
                .columns(columns));
    }
    ...
    public void writeParquetPartitioned(String root, String... partitionBy) throws Exception {
        ParquetAdvanced.writePartitioned(this, root, partitionBy);
    }
    ...
    public static DataFrame readParquetHive(String root, String... partitionKeys) throws Exception {
        return ParquetAdvanced.scanHive(root, partitionKeys);
    }

    public static DataFrame readParquetHive(String root, String[] partitionKeys,
            ParquetAdvanced.ReadOptions opt) throws Exception {
        return ParquetAdvanced.scanHive(root, partitionKeys, opt);
    }

    /** Stream advanced parquet batches (column prune + filter). */
    public static long streamParquetAdvanced(String path,
            ParquetAdvanced.ReadOptions opt,
            java.util.function.Consumer<DataFrame> consumer) throws Exception {
        return ParquetAdvanced.stream(path, opt, consumer);
    }
    ...
    public Rolling rolling(int window) {
        return new Rolling(this, window);
    }

    public Rolling rolling(int window, int minPeriods) {
        return new Rolling(this, window, minPeriods);
    }

    public Expanding expanding() {
        return new Expanding(this);
    }

    public Expanding expanding(int minPeriods) {
        return new Expanding(this, minPeriods);
    }

    public Ewm ewm(double alpha) {
        return new Ewm(this, alpha);
    }

    public Ewm ewm(double alpha, boolean adjust) {
        return new Ewm(this, alpha, adjust);
    }

    /** Feature-engineering façade entry. */
    public FeatureEngineering feature() {
        return new FeatureEngineering(this);
    }
     ...
     * </pre>
     */
    public DataFramePipeline pipeline() {
        return new DataFramePipeline(this);
    }
     ...
     * </pre>
     */
    public DataFrameDataset.Builder toDataset() {
        return DataFrameDataset.builder(this);
    }
     ...
     * </pre>
     */
    public DataFramePlot plot() {
        return new DataFramePlot(this);
    }
    ...
    public static DataFrame fromVectors(String vectorCol, float[][] data) {
        return VectorColumn.fromVectors(vectorCol, data);
    }

    public static DataFrame fromVectors(String vectorCol, float[][] data, String idCol, long[] ids) {
        return VectorColumn.fromVectors(vectorCol, data, idCol, ids);
    }
    ...
    public DataFrame annSearch(String vectorCol, float[] query, int k, int efSearch,
                               Distance space) throws Exception {
        HnswIndex idx = buildHnsw(vectorCol)
            .space(space)
            .build();
        AnnSearchResult r = idx.search(query, k, efSearch);
        return annResultToFrame(r);
    }

    private DataFrame annResultToFrame(AnnSearchResult r) {
    ...
    public static final class HnswBuild {
        ...
        public HnswBuild space(Distance d) {
        ...
        public HnswIndex build() {
            ...
            float[] matrix = VectorColumn.pack(col);
            return HnswIndex.builder(dim)
                .M(M)
    ...
    // ---- FAISS (dataframe.faiss, API-aligned with Python faiss) ----

    /**
     * Fluent FAISS index builder bound to a VECTOR column.
     * <pre>
     *   Index idx = df.buildFaiss("emb").hnsw(32).efConstruction(128)
     *       .metric(MetricType.METRIC_INNER_PRODUCT).ids("id").build();
     *   DataFrame hits = df.faissSearch(idx, query, 10);
     * </pre>
     */
    public FaissBuild buildFaiss(String vectorCol) {
        return new FaissBuild(this, vectorCol);
    }

    /**
     * Search with a pre-built FAISS {@link Index}; returns hit rows plus
     * {@code _distance}, {@code _rank}, and {@code _id} (external id when present).
     */
    public DataFrame faissSearch(Index index, float[] query, int k) {
        ...
        SearchResult r = index.search(query, k);
        return faissResultToFrame(r, 0);
    }

    /** Batch queries → list of hit frames (one per query). */
    public List<DataFrame> faissSearch(Index index, float[][] queries, int k) {
        if (index == null) throw new IllegalArgumentException("index is null");
        SearchResult r = index.search(queries, k);
        List<DataFrame> out = new ArrayList<>(r.nq());
        for (int q = 0; q < r.nq(); q++) out.add(faissResultToFrame(r, q));
        return out;
    }

    /**
     * Ephemeral HNSW build + search on {@code vectorCol} (FAISS path, analogous to
     * {@link #annSearch(String, float[], int)}).
     */
    public DataFrame faissSearch(String vectorCol, float[] query, int k) {
        Index idx = buildFaiss(vectorCol).hnsw(16).efConstruction(100).build();
        return faissSearch(idx, query, k);
    }

    private DataFrame faissResultToFrame(SearchResult r, int q) {
        long[] ids = r.I[q];
        float[] dists = r.D[q];
        DataFrame out = DataFrame.create();
        for (Column c : columns) out.addColumn(c.name(), c.dtype());
        out.addColumn("_distance", Column.DType.FLOAT64);
        out.addColumn("_rank", Column.DType.INT64);
        out.addColumn("_id", Column.DType.INT64);
        int n = ids == null ? 0 : ids.length;
        for (int i = 0; i < n; i++) {
            long id = ids[i];
            if (id < 0) continue;
            int row = out.addEmptyRow();
            // If id looks like a row index into this frame, copy columns
            if (id < rowCount()) {
                int ri = (int) id;
                for (Column c : columns) out.set(row, c.name(), c.get(ri));
            }
            out.set(row, "_distance", (double) dists[i]);
            out.set(row, "_rank", (long) (i + 1));
            out.set(row, "_id", id);
    ...
    /** Fluent builder returned by {@link #buildFaiss(String)}. */
    public static final class FaissBuild {
        public enum Kind { HNSW, FLAT_L2, FLAT_IP, IVFPQ }

        private final DataFrame df;
        private final String vectorCol;
        private Kind kind = Kind.HNSW;
        private int M = 16;
        private int efConstruction = 200;
        private int efSearch = 64;
        private MetricType metric = MetricType.METRIC_L2;
        private boolean normalize = false;
        private String idCol = null;
        // IVFPQ
        private int nlist = 256;
        private int pqM = 16;
        private int nbits = 8;
        private int nprobe = 16;

        FaissBuild(DataFrame df, String vectorCol) {
            this.df = df;
            this.vectorCol = vectorCol;
        }

        public FaissBuild hnsw(int M) { this.kind = Kind.HNSW; this.M = M; return this; }
        public FaissBuild flatL2() { this.kind = Kind.FLAT_L2; this.metric = MetricType.METRIC_L2; return this; }
        public FaissBuild flatIP() { this.kind = Kind.FLAT_IP; this.metric = MetricType.METRIC_INNER_PRODUCT; return this; }
        public FaissBuild ivfpq(int nlist, int m, int nbits) {
            this.kind = Kind.IVFPQ; this.nlist = nlist; this.pqM = m; this.nbits = nbits; return this;
        }
        public FaissBuild efConstruction(int ef) { this.efConstruction = ef; return this; }
        public FaissBuild efSearch(int ef) { this.efSearch = ef; return this; }
        public FaissBuild metric(MetricType m) { this.metric = m; return this; }
        public FaissBuild normalize(boolean v) { this.normalize = v; return this; }
        public FaissBuild ids(String idCol) { this.idCol = idCol; return this; }
        public FaissBuild nprobe(int n) { this.nprobe = n; return this; }
        ...
        public Index build() {
            ...
            float[] matrix = VectorColumn.pack(col);
            int n = col.size();
            if (normalize) Faiss.normalize_L2(matrix, n, dim);

            long[] extIds = null;
            if (idCol != null && df.hasColumn(idCol)) {
                Column idc = df.column(idCol);
                extIds = new long[n];
                for (int i = 0; i < n; i++) {
                    Object v = idc.get(i);
                    extIds[i] = v instanceof Number ? ((Number) v).longValue() : i;
                }
            }

            Index raw;
            switch (kind) {
                case FLAT_L2 -> raw = new IndexFlatL2(dim);
                case FLAT_IP -> raw = new IndexFlatIP(dim);
                case IVFPQ -> {
                    Index quant = metric == MetricType.METRIC_INNER_PRODUCT
                        ? new IndexFlatIP(dim) : new IndexFlatL2(dim);
                    IndexIVFPQ ivf = new IndexIVFPQ(quant, dim, nlist, pqM, nbits);
                    ivf.metric_type = metric;
                    ivf.train(matrix, n);
                    ivf.nprobe = nprobe;
                    raw = ivf;
                }
                default -> {
                    IndexHNSWFlat h = new IndexHNSWFlat(dim, M, metric);
                    h.hnsw.efConstruction = efConstruction;
            ...
            if (extIds != null) {
                // FAISS IndexIDMap: wrap empty inner, add_with_ids → inner.add + id translation
                IndexIDMap map = new IndexIDMap(raw);
                map.add_with_ids(matrix, n, extIds);
                return map;
            }
            raw.add(matrix, n);
            return raw;
    ...
    /**
     * Push VECTOR/EMBEDDING column rows into a {@link VectorStore}.
     * <pre>
     ...
     * </pre>
     */
    public void toVectorStore(VectorStore store,
     ...
     */
    public static DataFrame vectorSearch(VectorStore store,
    ...
    /** Alias for {@link #vectorSearch(VectorStore, float[], int)}. */
     ...
     * VectorStores#open(String, java.util.Map)}).
     */
    public static VectorStore openVectorStore(
            String scheme, java.util.Map<String, Object> config) {
        return VectorStores.open(scheme, config);
    }
     ...
     */
    public static VectorStore openVectorStore(String uri) {
        return VectorStores.open(uri);
    }
     ...
     * }</pre>
     */
    public VectorStore toMemoryVectorStore(
    ...
    public VectorStore toMemoryVectorStore(
        ...
        return toMemoryVectorStore(name, idCol, vectorCol, dim,
                VectorMetric.COSINE, payloadCols);
    }
    ...
    public VectorStore toRedisVectorStore(
    ...
    public VectorStore toRedisVectorStore(
            ...
            java.time.Duration ttl, String... payloadCols) {
        return toRedisVectorStore(host, port, index, idCol, vectorCol, dim,
                VectorMetric.COSINE, ttl, payloadCols);
    }

    /**
     * Upsert this DataFrame's vectors into an existing {@link
     * RedisVectorStore} with TTL.
     */
    ...
    public VectorStore toOpenSearch(
    ...
    public VectorStore toVectorStore(
            ...
            String idCol, String vectorCol, String... payloadCols) {
        VectorStore vs =
                VectorStores.open(scheme, config);
        vs.ensureCollection();
        toVectorStore(vs, idCol, vectorCol, payloadCols);
        return vs;
    }
    ...
    /**
     * Write this DataFrame via the full {@link Milvus}
     * client (REST v2, no milvus-sdk-java). Mirrors Redis-style {@code toRedis}.
     */
    public int toMilvus(Milvus milvus,
                        MilvusOptions options) {
        return milvus.writeDataFrame(this, options);
    }

    public int toMilvus(Milvus milvus, String collection) {
        return toMilvus(milvus, MilvusOptions.collection(collection));
    }
    ...
    public int toMilvus(String milvusUri,
                        MilvusOptions options) {
        try (Milvus m =
                     Milvus.connectUri(milvusUri)) {
            return toMilvus(m, options);
        }
    }

    public static DataFrame readMilvus(Milvus milvus,
                                       MilvusOptions options) {
    ...
    public static DataFrame readMilvus(Milvus milvus,
                                       String collection) {
        return readMilvus(milvus, MilvusOptions.collection(collection));
    }
    ...
    public static DataFrame searchMilvus(Milvus milvus,
                                         float[] query, int topK,
                                         MilvusOptions options) {
    ...
    /**
     * Write via full {@link OpenSearch} client
     * (REST + _bulk, no opensearch-java).
     */
    public int toOpenSearch(OpenSearch client,
                            OpenSearchOptions options) {
        return client.writeDataFrame(this, options);
    }

    public int toOpenSearch(OpenSearch client, String index) {
        return toOpenSearch(client, OpenSearchOptions.index(index));
    }

    public int toOpenSearch(String openSearchUri,
                            OpenSearchOptions options) {
        try (OpenSearch os =
                     OpenSearch.connectUri(openSearchUri)) {
            return toOpenSearch(os, options);
        }
    }

    public static DataFrame readOpenSearch(OpenSearch client,
                                           OpenSearchOptions options) {
    ...
    public static DataFrame readOpenSearch(OpenSearch client,
                                           String index) {
        return readOpenSearch(client, OpenSearchOptions.index(index));
    }
    ...
    public static DataFrame searchOpenSearch(OpenSearch client,
                                             float[] query, int topK,
                                             OpenSearchOptions options) {
    ...
    /**
     * Write via full {@link Mongo} client
     * (Atlas Data API; official driver via {@code MongoBackend} SPI).
     */
    public int toMongo(Mongo mongo,
    ...
    public int toMongo(Mongo mongo,
        ...
        return toMongo(mongo, MongoOptions.collection(database, collection));
    }

    public int toMongo(String mongoUri,
                       MongoOptions options) {
        try (Mongo m =
                     Mongo.connectUri(mongoUri)) {
            return toMongo(m, options);
        }
    }

    public static DataFrame readMongo(Mongo mongo,
                                      MongoOptions options) {
    ...
    public static DataFrame readMongo(Mongo mongo,
                                      String database, String collection) {
        return readMongo(mongo, MongoOptions.collection(database, collection));
    }
    ...
    public static DataFrame searchMongo(Mongo mongo,
                                        float[] query, int topK,
                                        MongoOptions options) {
    ...
    /**
     * Write via full {@link PgVector} client
     * (JDBC + pgvector; driver on app classpath).
     */
    public int toPgVector(PgVector pg,
                          PgVectorOptions options) {
        return pg.writeDataFrame(this, options);
    }

    public int toPgVector(PgVector pg, String table) {
        return toPgVector(pg, PgVectorOptions.table(table));
    }

    public int toPgVector(String jdbcUri,
                          PgVectorOptions options) {
        try (PgVector pg =
                     PgVector.connectUri(jdbcUri)) {
            return toPgVector(pg, options);
        }
    }

    public static DataFrame readPgVector(PgVector pg,
                                         PgVectorOptions options) {
    ...
    public static DataFrame readPgVector(PgVector pg,
                                         String table) {
        return readPgVector(pg, PgVectorOptions.table(table));
    }
    ...
    public static DataFrame searchPgVector(PgVector pg,
                                           float[] query, int topK,
                                           PgVectorOptions options) {
        ...
        switch (dtype) {
            ...
            case JSON:
                ...
                g.add(name, val instanceof String ? (String) val
                    : ComplexCellCodec.encodeText(val));
                break;
            default:
    ...
    private static void writeMapField(org.apache.parquet.example.data.simple.SimpleGroup g,
        ...
        java.util.Map<String, Object> map =
            ComplexCellCodec.asStringMap(val);
        if (map == null) return;
        for (java.util.Map.Entry<String, Object> e : map.entrySet()) {
            ...
            if (v instanceof Number || v instanceof Boolean || v instanceof String) {
                ...
                kv.add(1, ComplexCellCodec.encodeText(v));
            }
        }
    }
    ...
    private static void writeStructField(org.apache.parquet.example.data.simple.SimpleGroup g,
        ...
        for (int i = 0; i < gt.getFieldCount(); i++) {
            ...
            if (v instanceof Number || v instanceof Boolean || v instanceof String) {
                ...
                st.add(fname, ComplexCellCodec.encodeText(v));
            }
        }
