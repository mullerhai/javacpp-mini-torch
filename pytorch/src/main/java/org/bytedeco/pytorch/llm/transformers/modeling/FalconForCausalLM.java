...
/**
 * Falcon causal LM. Delegates to an existing Llama-style backbone.
 */
@Properties(inherit = org.bytedeco.pytorch.presets.torch.class)
public class FalconForCausalLM extends Module {
    ...
    public FalconForCausalLM(PretrainedConfig config) {
        super("FalconForCausalLM");
        this.config = Objects.requireNonNull(config);
        this.backbone = null; // TODO: wire real Falcon backbone
        this.lm_head = register_module("lm_head",
                new LinearImpl(new LinearOptions(config.hiddenSize(), config.vocabSize()).bias(false)));
    }

    public static FalconForCausalLM fromConfig(PretrainedConfig config) {
        return new FalconForCausalLM(config);
    }
    ...
    public Tensor forward(Tensor inputIds) {
        throw new UnsupportedOperationException("Falcon causal LM forward not yet implemented");
    }
