...
/**
 * BLOOM causal LM. Delegates to an existing Llama-style backbone.
 */
@Properties(inherit = org.bytedeco.pytorch.presets.torch.class)
public class BloomForCausalLM extends Module {
    ...
    public BloomForCausalLM(PretrainedConfig config) {
        super("BloomForCausalLM");
        this.config = Objects.requireNonNull(config);
        this.backbone = null; // TODO: wire real BLOOM backbone
        this.lm_head = register_module("lm_head",
                new LinearImpl(new LinearOptions(config.hiddenSize(), config.vocabSize()).bias(false)));
    }

    public static BloomForCausalLM fromConfig(PretrainedConfig config) {
        return new BloomForCausalLM(config);
    }
    ...
    public Tensor forward(Tensor inputIds) {
        throw new UnsupportedOperationException("BLOOM causal LM forward not yet implemented");
    }
