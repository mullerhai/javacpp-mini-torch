...
import org.bytedeco.pytorch.llm.transformers.modeling.ClipSegForImageSegmentation;
...
/**
 * HF {@code AutoModelForImageSegmentation.from_pretrained} entry point.
 * Routes to ClipSeg, GroupViT, SegGpt.
 */
public final class AutoModelForImageSegmentation {

    private AutoModelForImageSegmentation() {}
    ...
    public static Bundle fromDirectory(Path dir) throws IOException {
        ...
        String mt = cfg.modelType() == null ? "clipseg" : cfg.modelType().name().toLowerCase();
        ...
        Module model = ClipSegForImageSegmentation.fromConfig(cfg);
