...
import org.bytedeco.pytorch.llm.transformers.modeling.DptForDepthEstimation;
...
/**
 * HF {@code AutoModelForDepthEstimation.from_pretrained} entry point.
 * Routes to DPT, DepthAnything, ZoeDepth, GLPN.
 */
public final class AutoModelForDepthEstimation {

    private AutoModelForDepthEstimation() {}
    ...
    public static Bundle fromDirectory(Path dir) throws IOException {
        ...
        String mt = cfg.modelType() == null ? "dpt" : cfg.modelType().name().toLowerCase();
        ...
        Module model = DptForDepthEstimation.fromConfig(cfg);
