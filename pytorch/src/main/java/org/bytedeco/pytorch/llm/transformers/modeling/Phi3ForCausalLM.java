...
/**
 * Phi-3 causal LM. Thin wrapper delegating to an existing Llama-style backbone.
 */
@Properties(inherit = org.bytedeco.pytorch.presets.torch.class)
public class Phi3ForCausalLM extends Module {
    ...
    public Phi3ForCausalLM(PretrainedConfig config) {
        super("Phi3ForCausalLM");
        this.config = Objects.requireNonNull(config);
        this.backbone = null; // TODO: wire real Phi3 backbone
        this.lm_head = register_module("lm_head",
                new LinearImpl(new LinearOptions(config.hiddenSize(), config.vocabSize()).bias(false)));
    }

    public static Phi3ForCausalLM fromConfig(PretrainedConfig config) {
        return new Phi3ForCausalLM(config);
    }
    ...
    public Tensor forward(Tensor inputIds) {
        throw new UnsupportedOperationException("Phi-3 causal LM forward not yet implemented");
    }
