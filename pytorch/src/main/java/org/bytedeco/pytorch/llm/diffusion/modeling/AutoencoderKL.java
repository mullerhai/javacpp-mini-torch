...
public class AutoencoderKL extends Module {
    ...
    public static class ResBlock extends Module {
        ...
        public ResBlock(int channels) {
            super("ResBlock");
            this.norm1 = register_module("norm1", new GroupNormImpl(channels, channels, 32));
            this.act = register_module("act", new SiLUImpl());
            this.conv1 = register_module("conv1", conv2d(channels, channels, 3, 1));
            this.norm2 = register_module("norm2", new GroupNormImpl(channels, channels, 32));
    ...
    static Conv2dImpl conv2d(int inCh, int outCh, int kernel, int stride) {
        return new Conv2dImpl(
            new Conv2dOptions(inCh, outCh, kernel).stride(stride).padding(kernel / 2).bias(false));
    }
    ...
    static ConvTranspose2dImpl convTranspose2d(int inCh, int outCh, int kernel, int stride, int pad) {
        return new ConvTranspose2dImpl(
            new ConvTranspose2dOptions(inCh, outCh, kernel).stride(stride).padding(pad));
    }
    ...
    public static class Encoder extends Module {
        ...
        public Encoder(int inChannels, int latentChannels, int[] ch) {
            ...
            for (int i = 0; i < ch.length - 1; i++) {
                downBlocks.add(register_module("down_blocks." + i,
                    new DownBlock(ch[i], ch[i + 1])));
            }

            register_module("mid.res1", new ResBlock(ch[ch.length - 1]));
            register_module("mid.norm", new GroupNormImpl(ch[ch.length - 1], ch[ch.length - 1], 32));
            register_module("mid.act", new SiLUImpl());
            register_module("mid.conv", conv2d(ch[ch.length - 1], ch[ch.length - 1], 3, 1));
        }
        ...
        public Tensor forward(Tensor x) {
            ...
            h = ((ResBlock) get_submodule("mid.res1")).forward(h);
            h = ((GroupNormImpl) get_submodule("mid.norm")).forward(h);
            h = gelu(h);
            h = ((Conv2dImpl) get_submodule("mid.conv")).forward(h);
            return h;
        }
    }
    ...
    public static class Decoder extends Module {
        ...
        public Decoder(int inChannels, int latentChannels, int[] ch) {
            ...
            register_module("mid.res1", new ResBlock(ch[0]));
            register_module("mid.norm", new GroupNormImpl(ch[0], ch[0], 32));
            register_module("mid.act", new SiLUImpl());
            register_module("mid.conv", conv2d(ch[0], ch[0], 3, 1));

            for (int i = 0; i < ch.length - 1; i++) {
                upBlocks.add(register_module("up_blocks." + i,
                    new UpBlock(ch[i], ch[i + 1])));
            }

            register_module("conv_out", conv2d(ch[ch.length - 1], inChannels, 3, 1));
        }
        ...
        public Tensor forward(Tensor z) {
            ...
            h = ((Conv2dImpl) get_submodule("conv_out")).forward(gelu(h));
            return h;
        }
    }
    ...
    public Tensor encodeAndSample(Tensor imageTensor) {
        ...
        return mean.add(torch.randn_like(mean).mul(exp(logvar.mul(0.5))));
    }
    ...
    public Tensor klLoss(Tensor imageTensor) {
        ...
        Tensor mean = ((Conv2dImpl) quantConv).forward(h);
        Tensor logvar = ((Conv2dImpl) logSigConv).forward(h);
        return logvar.add(1.0).sub(mean.pow(2.0)).sub(exp(logvar)).neg().sub(1.0).mul(0.5).sum().div(mean.size(0));
    }
