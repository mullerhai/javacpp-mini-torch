...
/**
 * Qwen3 causal LM with HuggingFace-identical parameter names.
 *
 * <p>Differs from {@link Qwen2ForCausalLM} primarily in attention:
 * <ul>
 *   <li>No q/k/v bias ({@code attention_bias=false})</li>
 *   <li>QK-Norm: {@code self_attn.q_norm}/{@code k_norm} RMSNorm over {@code head_dim}</li>
 *   <li>Explicit {@code head_dim} (may differ from {@code hidden_size / num_attention_heads})</li>
 * </ul>
 *
 * <pre>
...
public class Qwen3ForCausalLM extends Module {
    ...
    public Qwen3ForCausalLM(PretrainedConfig config) {
        super("Qwen3ForCausalLM");
        this.config = Objects.requireNonNull(config, "config");
        this.model = register_module("model", new Qwen3Model(config));
    ...
    public static Qwen3ForCausalLM fromConfig(PretrainedConfig config) {
        return new Qwen3ForCausalLM(config);
    }
    ...
    @Properties(inherit = org.bytedeco.pytorch.presets.torch.class)
    public static class Qwen3Model extends Module {
        static { Loader.load(org.bytedeco.pytorch.presets.torch.class); }

        public final EmbeddingImpl embed_tokens;
        public final List<Qwen3DecoderLayer> layers = new ArrayList<>();
        public final RMSNorm norm;
        private final PretrainedConfig config;

        public Qwen3Model(PretrainedConfig config) {
            super("Qwen3Model");
            this.config = config;
            this.embed_tokens = register_module("embed_tokens",
                    new EmbeddingImpl(config.vocabSize(), config.hiddenSize()));
            for (int i = 0; i < config.numHiddenLayers(); i++) {
                layers.add(register_module("layers/" + i, new Qwen3DecoderLayer(config, i)));
            }
        ...
        public Tensor forward(Tensor inputIds) {
            ...
            for (Qwen3DecoderLayer layer : layers) {
                x = layer.forward(x);
            }
    ...
    @Properties(inherit = org.bytedeco.pytorch.presets.torch.class)
    public static class Qwen3DecoderLayer extends Module {
        static { Loader.load(org.bytedeco.pytorch.presets.torch.class); }
        ...
        public Qwen3DecoderLayer(PretrainedConfig cfg, int layerIdx) {
            super("Qwen3DecoderLayer" + layerIdx);
