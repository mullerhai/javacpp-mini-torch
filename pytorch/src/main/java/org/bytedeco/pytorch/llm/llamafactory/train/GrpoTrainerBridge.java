...
/** GRPO stage bridge → {@link GRPOTrainer}. */
public final class GrpoTrainerBridge {
    private GrpoTrainerBridge() {}

    public static GRPOTrainer create(FactoryArgs args, LoadedModel loaded, int maxSteps) {
        BaseTrainer t = TrainerFactory.create(args, loaded, maxSteps);
        if (t instanceof GRPOTrainer grpo) {
            return grpo;
        }
        Optimizer opt = TrainerFactory.buildOptimizer(args, loaded);
        GRPOConfig cfg = TrainerFactory.grpoConfig(args, maxSteps);
        LlmForward fwd = TrainerFactory.causalForward(loaded);
        return new GRPOTrainer(loaded.module(), fwd, null, null, opt, cfg);
    }
}
