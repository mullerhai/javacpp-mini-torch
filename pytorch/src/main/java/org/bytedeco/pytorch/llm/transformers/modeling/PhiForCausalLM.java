...
/**
 * Phi causal LM. Thin wrapper delegating to an existing backbone.
 */
@Properties(inherit = org.bytedeco.pytorch.presets.torch.class)
public class PhiForCausalLM extends Module {
    ...
    public PhiForCausalLM(PretrainedConfig config) {
        super("PhiForCausalLM");
        this.config = Objects.requireNonNull(config);
        this.backbone = null; // TODO: wire real Phi backbone
        this.lm_head = register_module("lm_head",
                new LinearImpl(new LinearOptions(config.hiddenSize(), config.vocabSize()).bias(false)));
    }

    public static PhiForCausalLM fromConfig(PretrainedConfig config) {
        return new PhiForCausalLM(config);
    }
    ...
    public Tensor forward(Tensor inputIds) {
        throw new UnsupportedOperationException("Phi causal LM forward not yet implemented");
    }
