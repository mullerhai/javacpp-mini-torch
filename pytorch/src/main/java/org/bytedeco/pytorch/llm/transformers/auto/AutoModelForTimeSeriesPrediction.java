...
import org.bytedeco.pytorch.llm.transformers.modeling.TimesFMForTimeSeriesPrediction;
...
/**
 * HF {@code AutoModelForTimeSeriesPrediction.from_pretrained} entry point.
 * Routes to TimesFM, Informer, Autoformer.
 */
public final class AutoModelForTimeSeriesPrediction {

    private AutoModelForTimeSeriesPrediction() {}
    ...
    public static final class Bundle {
        ...
        public Bundle(Module model, PretrainedConfig config, Path snapshot,
                      WeightLoader.LoadReport loadReport, String modelType) {
            this.model = model;
            this.config = config;
    ...
    public static Bundle fromDirectory(Path dir) throws IOException {
        ...
        String mt = cfg.modelType() == null ? "timesfm" : cfg.modelType().name().toLowerCase();
        Module model = TimesFMForTimeSeriesPrediction.fromConfig(cfg);
        WeightLoader.LoadReport rep = WeightLoader.loadAndBind(model, dir);
        return new Bundle(model, cfg, dir, rep, mt);
    }
}
