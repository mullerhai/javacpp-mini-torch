...
import org.bytedeco.pytorch.llm.transformers.modeling.SuperPointForKeypointDetection;
...
/**
 * HF {@code AutoModelForKeypointDetection.from_pretrained} entry point.
 * Routes to SuperPoint.
 */
public final class AutoModelForKeypointDetection {

    private AutoModelForKeypointDetection() {}
    ...
    public static Bundle fromDirectory(Path dir) throws IOException {
        ...
        String mt = cfg.modelType() == null ? "superpoint" : cfg.modelType().name().toLowerCase();
        ...
        Module model = SuperPointForKeypointDetection.fromConfig(cfg);
