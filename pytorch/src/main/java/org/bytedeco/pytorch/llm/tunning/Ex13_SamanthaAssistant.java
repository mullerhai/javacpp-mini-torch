...
/** Ex13 — Samantha assistant /samantha-1.1 dataset QLoRA fine-tuning. */
public final class Ex13_SamanthaAssistant {
    public static final String NAME = "Ex13_SamanthaAssistant";

    public static void run(FastTokenizer tokenizer) throws IOException {
        TunningSupport.banner(13, "Samantha assistant");
        File out = new File("build/ex13_outputs"); out.mkdirs();
        FastTokenizer tok = AutoTokenizer.fromPretrainedTokenizerOnly("OpenAssistant/oasst-llama-7b");
        ...
        java.util.List<Map<String, Object>> trainData =
                TunningSupport.loadDataset("Locutusque/samantha-1.1", null, "train", 1024);
        java.util.List<Map<String, Object>> formattedData = trainData.stream().map(row -> {
            String t = TunningSupport.samanthaPrompt(TunningSupport.chatMessages(
                    "system", "You are Samantha, a sentient AI.",
    ...
    public static void main(String[] args) throws IOException { run(TunningSupport.tokenizerFor("OpenAssistant/oasst-llama-7b")); }
}
