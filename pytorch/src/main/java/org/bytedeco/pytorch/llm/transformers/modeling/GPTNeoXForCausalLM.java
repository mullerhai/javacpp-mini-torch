...
/**
 * GPT-NeoX causal LM. Delegates to an existing backbone.
 */
@Properties(inherit = org.bytedeco.pytorch.presets.torch.class)
public class GPTNeoXForCausalLM extends Module {
    ...
    public GPTNeoXForCausalLM(PretrainedConfig config) {
        super("GPTNeoXForCausalLM");
        this.config = Objects.requireNonNull(config);
        this.backbone = null; // TODO: wire real GPT-NeoX backbone
        this.lm_head = register_module("lm_head",
                new LinearImpl(new LinearOptions(config.hiddenSize(), config.vocabSize()).bias(false)));
    }

    public static GPTNeoXForCausalLM fromConfig(PretrainedConfig config) {
        return new GPTNeoXForCausalLM(config);
    }
    ...
    public Tensor forward(Tensor inputIds) {
        throw new UnsupportedOperationException("GPT-NeoX causal LM forward not yet implemented");
    }
