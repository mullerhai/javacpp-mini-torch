...
/** ORPO stage bridge → {@link ORPOTrainer}. */
public final class OrpoTrainerBridge {
    private OrpoTrainerBridge() {}

    public static ORPOTrainer create(FactoryArgs args, LoadedModel loaded, int maxSteps) {
        BaseTrainer t = TrainerFactory.create(args, loaded, maxSteps);
        if (t instanceof ORPOTrainer orpo) {
            return orpo;
        }
        Optimizer opt = TrainerFactory.buildOptimizer(args, loaded);
        ORPOConfig cfg = TrainerFactory.orpoConfig(args, maxSteps);
        LlmForward fwd = TrainerFactory.causalForward(loaded);
        return new ORPOTrainer(loaded.module(), fwd, opt, cfg);
    }
}
