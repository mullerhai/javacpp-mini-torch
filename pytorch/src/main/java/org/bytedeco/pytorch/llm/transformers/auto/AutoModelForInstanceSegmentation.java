...
import org.bytedeco.pytorch.llm.transformers.modeling.MaskFormerForInstanceSegmentation;
...
/**
 * HF {@code AutoModelForInstanceSegmentation.from_pretrained} entry point.
 * Routes to MaskFormer.
 */
public final class AutoModelForInstanceSegmentation {

    private AutoModelForInstanceSegmentation() {}
    ...
    public static Bundle fromDirectory(Path dir) throws IOException {
        ...
        String mt = cfg.modelType() == null ? "maskformer" : cfg.modelType().name().toLowerCase();
        ...
        Module model = MaskFormerForInstanceSegmentation.fromConfig(cfg);
